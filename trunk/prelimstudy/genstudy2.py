...
for (en,ch,line) in zip(enlist, chlist, range(1, len(enlist))):
    if (((ch.find(u'有') != -1 and (' '+en).find(' a ') != -1 and len(en) < 80) # 1448 (182 less than 40 in ch)
      or (ch.find(u'把') != -1 and len(en) < 80) # 538 (116 less than 40 in ch)
      or (ch.find(u'有') == 0) # and len(ch) < 40: # 174
      or (en.find('a ') == 0) # and len(ch) < 40: # 248
      or (ch.find(u'一') != -1 and (' '+en).find(' a ') != -1 and len(en) < 60))
...
sys.stdout = codecs.open('study3.txt', 'w', encoding='utf-8')
