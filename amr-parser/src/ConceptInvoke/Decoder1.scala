    ...
    def decode(input: Input) : DecoderResult = {
        ...
        for (i <- Range(0, sentence.size)) {
            ...
            for (concept <- conceptList) {
                ...
                val endpoint = i + concept._1.size - 1
                if (score >= 0 || bestState(endpoint) == None || bestState(endpoint).get._1 <= score) { // we use <= so that later concepts (i.e. ones our conceptTable) have higher priority
