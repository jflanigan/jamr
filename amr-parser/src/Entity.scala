...
object Entity {
    ...
    def entitiesFromConllBIO(conllStr: String, column: Int = 1) : Array[Entity] = {
        ...
        while (i < conll.size) {
            if (conll(i)(column) != "O") {
                assert(conll(i)(column).matches("(B|I|L|U)-.*"), "NER data is not in conll BIO tagging format")  // UIUC format
