...
object ExtractAbstractRules3xRs {

    val usage = """Usage: scala -classpath . edu.cmu.lti.nlp.amr.Generate.ExtractAbstractRules3xRs --dependencies deps_file < amr_file > outfile"""
    ...
    def main(args: Array[String]) {
        ...
        for { block <- Corpus.splitOnNewline(Source.stdin.getLines)
            ...
            extractRules(graph, graph.root, sentence, pos, spans, spanArray)
            logger(0,"")
            i += 1
        }
    }

    def extractRules(graph: Graph, node: Node, sent: Array[String], pos : Array[String], spans: Map[String, (Option[Int], Option[Int])], spanArray: Array[Boolean]) {
        ...
        for (child <- children) {
            extractRules(graph, child.node, sent, pos, spans, spanArray)
        }
    }
