...
object TrainRuleModel {

    val usage = """Usage: scala -classpath . edu.cmu.lti.nlp.amr.Generate.TrainRuleModel --training-data amr_file > outfile"""
    ...
    def main(args: Array[String]) {
        ...
        //ruleInventory.extractFromCorpus(io.Scala.fromFile(options('trainingData)).getLines, pos)

        //ruleInventory.save(options('output))

        val syntheticRules = new SyntheticRules.TrainObj(options)
        syntheticRules.train
    }
}
